...
class LadyController extends BaseController
{
    ...
    public function config(){
        try {
//            $types = Type::tree(['status' => Type::STATUS_1, 'parent_id' => 10002]);
            $config =  [
                'status' => Lady::STATUS_TEXT,
//                'types' => $types
            ];
    ...
    public function lock(RequestInterface $request)
    {
        try {
            ...
            Lady::store($ids, ['status' => Lady::STATUS_3]);
    ...
    public function active(RequestInterface $request)
    {
        try {
            ...
            Lady::store($ids, ['status' => Lady::STATUS_1]);
            return $this->returnJson(0 , null, '操作成功');
        } catch (\Throwable $e) {
            return $this->returnJson(1, null, $e->getMessage());
        }
    ...
    public function good(RequestInterface $request)
    {
        try {
            ...
            Lady::store($ids, ['status' => Lady::STATUS_2]);
            return $this->returnJson(0 , null, '操作成功');
        } catch (\Throwable $e) {
            return $this->returnJson(1, null, $e->getMessage());
        }
