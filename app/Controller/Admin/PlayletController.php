...
class PlayletController extends BaseController
{
    ...
    public function config(){
        try {
            $types = Type::tree(['status' => Type::STATUS_1, 'parent_id' => 10038]);
            $tags = Tag::tree([], 'name');
            $config =  [
                'status' => Playlet::STATUS_TEXT,
                'types' => $types,
                'tags' => $tags
            ];
    ...
    public function lock(RequestInterface $request)
    {
        try {
            ...
            Playlet::store($ids, ['status' => Playlet::STATUS_3]);
    ...
    public function active(RequestInterface $request)
    {
        try {
            ...
            Playlet::store($ids, ['status' => Playlet::STATUS_1]);
            return $this->returnJson(0 , null, '操作成功');
        } catch (\Throwable $e) {
            return $this->returnJson(1, null, $e->getMessage());
        }
    ...
    public function good(RequestInterface $request)
    {
        try {
            ...
            Playlet::store($ids, ['status' => Playlet::STATUS_2]);
            return $this->returnJson(0 , null, '操作成功');
        } catch (\Throwable $e) {
            return $this->returnJson(1, null, $e->getMessage());
        }
